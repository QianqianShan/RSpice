...
findSpice <- function(dylibpath, dylibname) {
    if (.Platform$OS.type == "unix") {
      ...
      if (is.null(dylibpath)) {
        # search across all directories to look for the shared object
        file.paths <- system(paste0("find ~/ -name ", dylibname), intern = TRUE)
        
      } else {
        if (dir.exists(dylibpath)) {
        file.paths <- system(paste0("find ", sub("/$", "" ,dylibpath) , .Platform$file.sep," -name ", dylibname), intern = TRUE)
        } else {
      ...
      if (is.null(dylibpath)) {
        file.paths <- system(paste0("cmd.exe /c dir /b/s", dylibname), intern = TRUE)
      } else {
        if (dir.exists(dylibpath)) {
          file.paths <- system(paste0("find ", sub("/$", "" ,dylibpath) , .Platform$file.sep," -name ", dylibname), intern = TRUE)
        } else {
        ...
        # file.paths <- system(paste0("cmd.exe /c dir /b/s ", sub("/$", "" ,dylibpath) ,
        #                             .Platform$file.sep,dylibname), intern = TRUE)
        cat(paste("\n Shared library exists in the following directories, the first directory
          is used. \n"))
        print(file.paths)
      }
